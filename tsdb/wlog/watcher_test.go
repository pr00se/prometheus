...
func TestReadToEndWithCheckpoint(t *testing.T) {
	...
	for _, compress := range compression.Types() {
		t.Run(fmt.Sprintf("compress=%s", compress), func(t *testing.T) {
			...
			Checkpoint(promslog.NewNopLogger(), w, 0, 1, func(_ chunks.HeadSeriesRef, _ int64) bool { return true }, 0)
			w.Truncate(1)
...
func TestReadCheckpoint(t *testing.T) {
	...
	for _, compress := range compression.Types() {
		t.Run(fmt.Sprintf("compress=%s", compress), func(t *testing.T) {
			...
			require.NoError(t, err)
			_, err = Checkpoint(promslog.NewNopLogger(), w, 30, 31, func(_ chunks.HeadSeriesRef, _ int64) bool { return true }, 0)
			require.NoError(t, err)
...
func TestCheckpointSeriesReset(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("compress=%s", tc.compress), func(t *testing.T) {
			...
			_, err = Checkpoint(promslog.NewNopLogger(), w, 2, 4, func(_ chunks.HeadSeriesRef, _ int64) bool { return true }, 0)
			require.NoError(t, err)
