...
func Checkpoint(logger *slog.Logger, w *WL, from, to int, keep func(id chunks.HeadSeriesRef, mint int64) bool, mint int64) (*CheckpointStats, error) {
	...
	for r.Next() {
		...
		case record.Metadata:
			...
			for _, m := range metadata {
				...
				// !!!! TODO -- we don't add this when we process a series record anymore, so what if we process a metadata before any other records for the series?
