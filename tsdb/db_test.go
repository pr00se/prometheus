...
func TestSizeRetention(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = wlog.Checkpoint(promslog.NewNopLogger(), db.Head().wal, first, last-1, func(_ chunks.HeadSeriesRef, _ int64) bool { return false }, 0)
	require.NoError(t, err)
	blockSize = int64(prom_testutil.ToFloat64(db.metrics.blocksBytes)) // Use the actual internal metrics.
	walSize, err = db.Head().wal.Size()
	require.NoError(t, err)
...
func TestMetadataCheckpointingOnlyKeepsLatestEntry(t *testing.T) {
	...
	require.NoError(t, err)
	keep := func(id chunks.HeadSeriesRef, _ int64) bool {
		return id != 3
	}
